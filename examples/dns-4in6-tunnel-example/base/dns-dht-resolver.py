...
class DHTResolver(BaseResolver):
    def __init__(
        self,
        tlds: List[str] = [".kira.internal."],
        upstream_resolver: Optional[str] = None,
    ):
        self.tlds = tlds
    ...
    def resolve_query(self, request: DNSRecord):
        ...
        if qtype not in [QTYPE.A, QTYPE.AAAA] or not any(
            qname.endswith(tld) for tld in self.tlds
        ):
        ...
        if not asked_present:
            logging.warning("Asked QTYPE %s not found but other QTYPES present")
            reply.header.rcode = RCODE.NXDOMAIN
    ...
    def resolve_update(self, request: DNSRecord):
        ...
        if not any(zname.endswith(tld) for tld in self.tlds):
            logging.warning("Rejecting update outside our authority: %s", request)
        ...
        for update in request.auth:
            if update.rtype not in [QTYPE.A, QTYPE.AAAA]:
                logging.warning(
                    "Rejecting update of RR %s: Only accepting updates to A and AAAA: %s",
                    update.rtype,
                    request,
                )
                reply.header.rcode = RCODE.FORMERR
            ...
            if not str(update.rname).endswith(zname):
                logging.warning("UNAME not matching ZNAME %s: %s", zname, update.rname)
                reply.header.rcode = RCODE.FORMERR
                return reply
    ...
    def resolve_upstream(self, request: DNSRecord) -> Optional[DNSRecord]:
        ...
        if self.upstream_resolver is None:
            logging.debug(
                "No upstream set. Can't ask upstream resolver for not internal address: %s",
                qname,
            )
            return None
        logging.info(
            "Request for %s is not for an internal address, asking upstream resolver.",
            qname,
        )
    ...
    def resolve_dht(self, domain: str) -> Optional[List[RR]]:
        ...
        try:
            fetch_r = requests.get(
                "http://localhost:8080/dht/fetch", params=fetch_params
            )
        ...
        if fetch_status >= 200 and fetch_status < 300:
            ...
            logging.error("Resolving service id %s not possible: %s", domain, fetch_r)
            return None
    ...
    def update_dht(self, rr: RR) -> bool:
        store_params = {"restore": "true", "key": self.domain_to_key(rr.rname)}
        ...
        store_r = requests.post(
            "http://localhost:8080/dht/store", params=store_params, data=rr_buffer.data
        )
        ...
        if store_status >= 200 and store_status < 300:
            ...
            logging.error("Announcing RR over DHT not possible: %s", store_r)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "tld",
        nargs="*",
        default=os.environ.get("DHTRESOLVER_TLD", "kira.internal.").split(":"),
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default=os.environ.get("DHTRESOLVER_UPSTREAM"),
        metavar="8.8.8.8",
    ...
    dnsserver = DNSServer(resolver, port=PORT, address="localhost", logger=dnslogger)
    try:
        dnsserver.start()
