...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--data", type=str, required=False, dest="data", help="Data to store at given node"
    )
    parser.add_argument(
        "--edges", type=argparse.FileType("r"), required=True, help="Path pointing to an edgelist"
    )
    parser.add_argument("--img", type=str, help="Docker image used by the node containers")
    parser.add_argument("--seed", type=str, required=False, help="Seed used to generate Node-IDs")
    parser.add_argument(
        "--plain",
        required=False,
        dest="plain",
    ...
    try:
        ...
        elif operation == "uln-table":
            result = network.get_node(args.node).uln_table()
