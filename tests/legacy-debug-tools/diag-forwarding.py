...
def parse_nft_output(output):
    errormsg = re.search(
        r'^(Error): Could not process rule: No such file or directory', output)
    if errormsg:
        return ""
    else:
        match = re.search(
            r'elements = { [a-fA-F0-9:]+ : ([a-fA-F0-9:]+) }', output)
        if match:
...
class TraceRouteInfo:
    def __init__(self, config):
        ...
        for node in self.topology.nodes:
            ...
            self.node_map_nodeid[normalize_ipv6_address(
                self.topology.nodes[node]["config"].ipv6)] = name
    ...
    def get_full_route(self, start_container_name, destaddr):
        ...
        while not finished:
            ...
            if re.search(r'^fcaa:', current_addr):
                result = get_fwtable_entry(
                    current_container_name, current_addr)
                if result:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Diagnose KIRA forwarding script')
