...
class R2KadNetwork:
    ...
    def _init_node(self, node: int):
        ...
        try:
            ...
            self._logger.debug(f"Adopting previous created container {container.name} for node {node}")
    ...
    def create_node_container(self, node: int, default_img: str, info=True) -> Container:
        ...
        img = default_img
        try:
            # to inject custom images on per node basis
            img = self.graph.nodes[node]["img"]
        except: 
            pass

        if img is None:
            logger.error("Couldn't determine the image to use for the node container!")
            return None
    ...
    def start(self, img: str = None) -> None:
        for (node, container) in self.graph.nodes(data="container"):
            if container is None:
                if img is None:
                    self._logger.warning(f"Couldn't start nor create container for node: {node}")
                    return
    ...
    def stop(self) -> None:
        for (node, container) in self.graph.nodes(data="container"):
            if container is None:
                self._logger.warning(f"Consider node without container stopped: {node}!")
                continue
    ...
    def connect_edge(self, origin: int, destination: int, network: Network = None, fallback_img: str = None) -> Network:
        if network is None:
            ...
            self._logger.info(f"Created new network for the connection: {network.name}")
        ...
        for node in [origin, destination]:
            try:
                ...
                if fallback_img is None:
                    self._logger.error(f"Couldn't create edge {origin}--{destination}, since {node} has no container")
                containers[node] = self.create_node_container(node, fallback_img, info=False)
                self._logger.info(f"Created node {node} to be able to connect {origin}--{destination}")
    ...
    def logs(self, node: int, follow=False) -> str:
        try:
            ...
            self._logger.error(f"Can't fetch logs at {node} without a container present!")
            return None
...
if __name__ == '__main__':
    ...
    parser.add_argument('operation', type=str, choices=["create", "connect", "start", "stop", "prune", "store", "fetch", "logs"])
